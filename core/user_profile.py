...
def build_mulberry_description(name: str, p: Personality, voice_gender: str, custom_vibe: str = "") -> str:
    """
    Generates the Silk mulberry voice description.
    Sent per-call to steer how the voice sounds.

    Design principle: only deviate from natural baseline when a trait is
    genuinely strong.  Balanced / default values add no modifiers so the
    voice stays natural and un-coached by default.
    """
    gender_base = "female" if voice_gender in ("female", "female_alt") else "male"

    # Only add modifiers for the non-default / strong end of each axis.
    # Default (chill, clean, punchy, sincere, english) → no modifier added,
    # so the voice stays natural.
    modifiers: list[str] = []

    if p.energy == "chaotic":
        modifiers.append("high-energy and fast-paced")

    if p.filter == "unfiltered":
        modifiers.append("bold and direct")

    if p.style == "dramatic":
        modifiers.append("expressive and dramatic")

    if p.tone == "sarcastic":
        modifiers.append("dry and sardonic")

    if p.lang_lean == "hindi":
        modifiers.append("heavily Hindi-accented Hinglish")
    else:
        modifiers.append("natural Hinglish")   # always name the accent baseline

    # Build description: natural baseline + only active modifiers
    if modifiers:
        modifier_str = ", ".join(modifiers)
        base = f"natural, conversational {gender_base} voice — {modifier_str}, like talking to a close friend"
    else:
        base = f"natural, conversational {gender_base} voice, like talking to a close friend"

    custom_vibe = custom_vibe.strip()
    if custom_vibe:
        return f"{base}; user's custom vibe: {custom_vibe}"
    return base
...
class UserProfileStore:
    ...
    def save(
        ...
        llm_prompt    = build_llm_system_prompt(name, personality, custom_vibe)
        mulberry_desc = build_mulberry_description(name, personality, voice_gender, custom_vibe)
