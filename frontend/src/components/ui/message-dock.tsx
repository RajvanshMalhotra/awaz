...
const defaultCharacters: Character[] = [
  ...
  { emoji: "🤖", name: "Robot", online: false, backgroundColor: "bg-red-300", gradientColors: "#fca5a5, #fef2f2" },
]
...
export function MessageDock({
  ...
  useEffect(() => {
    if (dockRef.current && !hasInitialized) {
      ...
      if (width > 0) { setCollapsedWidth(width); setHasInitialized(true) }
    }
  }, [hasInitialized])
  ...
  const hoverAnimation = shouldReduceMotion
    ? { scale: 1.02 }
    : { scale: 1.05, y: -8, transition: { type: "spring", stiffness: 400, damping: 25 } }

  const handleCharacterClick = (index: number) => {
    const character = characters[index]
    if (expandedCharacter === index) {
      setExpandedCharacter(null); setMessageInput(""); onDockToggle?.(false)
    } else {
      setExpandedCharacter(index); onCharacterSelect?.(character, index); onDockToggle?.(true)
    }
  }
  ...
  const handleSendMessage = () => {
    if (messageInput.trim() && expandedCharacter !== null) {
      ...
      setMessageInput("")
      if (closeOnSend) { setExpandedCharacter(null); onDockToggle?.(false) }
    }
  }
    ...
    >
      <motion.div
        ...
        transition={enableAnimations ? {
          type: "spring", stiffness: isExpanded ? 300 : 500, damping: isExpanded ? 30 : 35, mass: isExpanded ? 0.8 : 0.6,
      ...
      >
        <div className="flex items-center gap-2 relative">
          ...
          {showSparkleButton && (
            ...
            >
              <motion.button
                ...
                aria-label={characters[0]?.name || "Sparkle"}
              >
                <span className="text-2xl">{characters[0]?.emoji || "✨"}</span>
              </motion.button>
            </motion.div>
          )}
          ...
          {characters.slice(1, -1).map((character, index) => {
            ...
            return (
              <motion.div
                key={character.name || actualIndex}
              ...
              >
                ...
                >
                  ...
                  {/* Offline = red dot (when showOfflineIndicator is true) */}
                  {!character.online && showOfflineIndicator && (
                    <motion.div
            ...
            {isExpanded && (
              <motion.input
                ...
                onKeyDown={(e) => {
                  ...
                  if (e.key === "Escape" && closeOnEscape) { setExpandedCharacter(null); setMessageInput(""); onDockToggle?.(false) }
                }}
